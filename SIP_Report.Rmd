...
\subsection{First Faculty Effect on Final Cumulative GPA}
...
Course.Heights <- tibble(`Course Code` = unique(Filtered.Data3$First.Course)) %>%
                    arrange(`Course Code`) %>%
                      cbind(tibble(Height = seq(-0.35, 0.35, length.out = length(unique(Filtered.Data3$First.Course)))))
```
...
##statistics on the total number of students per faculty
mean.n <-  Double.Profs %>% 
            ungroup() %>% 
              pull(Num.Students) %>% 
               mean()
...
```{r Experiment One Reject/Accept}
...
```{r Full Experiment One Histogram, fig.cap = "Histogram of all mean Delta C's for experiment one, filled by effect polarity from the mean."}
...
```{r Doubly Significant Experiment One Histogram, fig.cap = "Histogram of doubly-significant mean Delta C's for experiment one, filled by effect polarity from the mean"}
##Histogram of doubly-significant mean professor effects onto final cumulative GPA for students that obeyed filtration assumptions.
Double.Profs.Data %>%
  mutate(Hist.Colors = factor(if_else(Mean.DeltaC < mean(pull(Double.Profs.Data, Mean.DeltaC)), "green", "red"))) %>%
      ggplot() +
...
```{r Experiment One DeltaC Context Plot, fig.cap = "Scatterplot of each doubly significant faculty's mean Delta C against the total number of students they taught from 2010 to 2017. Colored by effect polarity from the mean."}
                            ...
                            mutate(Point.Colors = if_else(Mean.DeltaC < mean(Double.Profs$Mean.DeltaC), "green", "red")) %>%
                              dplyr::select(First.Prof, Mean.DeltaC, Num.Students, Point.Colors) %>%
                                left_join(Overall.Num.Students, by = c("First.Prof" = "Faculty Random ID")) %>%
                                  ...
                                  scale_x_continuous(limits = c(0,3000), breaks = seq(0,3000, by = 250)) +
                                  theme(legend.position = "none") +
...
```{r Experiment One Sample Sizes, fig.cap = "Histogram and Boxplot of Doubly-Significant Sample Sizes for Experiment One."}
...
Double.Profs %>%
  ...
  scale_x_continuous() +
  labs(x = "Number of Students that Passed Filtration and Started with Faculty", y = "Count")
```

|     As the number of students trends towards zero, figure $18$ indicates that mean $\Delta C$ becomes more volatile. This volatility seems to evaporate past the $250$ student mark, down to a rough local average of $\pm 0.25$ for each polarity.  
...
##Scatterplot of sll course specific mean DeltaC's
Double.Courses %>%
  left_join(Course.Heights, by = c("First.Course" = "Course Code")) %>%
    ...
    theme(axis.text.y=element_blank(), axis.ticks.y=element_blank()) +
    labs(x = "Course's Doubly-Significant Mean \u0394C (GPA Points)", y = "Course", title = "Scatterplot of ALL Mean \u0394Cs for\nExperiment Two", color = "Course Code", size = "Number of Students")
...
Double.Courses %>%
  filter(T.Sig == TRUE, T.Sig == TRUE) %>%
    left_join(Course.Heights, by = c("First.Course" = "Course Code")) %>%
      ...
      scale_x_continuous(limits = c(-1, 1), breaks = seq(from = -1, to = 1, by = .5)) +
      theme(axis.text.y=element_blank(), axis.ticks.y=element_blank()) +
      labs(x = "Course's Doubly-Significant Mean \u0394C (GPA Points)", y = "Course", title = "Scatterplot of Doubly-Significant Mean \u0394Cs for\nExperiment Two", color = "Course Code", size = "Number of Students")
