...
It is impossible to differentiate graduate student instructors from full professors based on the random ID's. To avoid being rude, in the following analysis I will refer to an individual faculty member interchangeably as a faculty member or as a professor to avoid repitition, and to grant the anonymous person the highest possible title they might possess. 
...
```{r}
##Bootstrap for each faculty member that taught students that passed filtration assumptions
Boot.Data <- Filtered.Data5 %>%
              ungroup() %>%
                group_by(`Student Random ID`) %>%
                  slice(n()) %>%
                    dplyr::select(`Student Random ID`, Delta2, First.Prof, First.Course)

##Initialize list of faculty that passed filtration conditions
Filtered.Profs <- unique(Boot.Data$First.Prof)

##Record mean Delta2 over all students that passed filtration conditions.
global.mean <- mean(Boot.Data$Delta2)


##Create bootstrapper function which creates a bootstrap distribution for a given faculty member
make.overall.bootstrap.means <- function(prof.num) {
  og.sample <- Boot.Data$Delta2[Boot.Data$First.Prof == prof.num]
  ...
  boot.means <- tibble(Delta2.Boot.Means = boot.means)
  return(boot.means)
}
 
##Create observed mean grabber for 
get.observed.mean <- function(prof.num){
  observed.delta2s <- Boot.Data %>%
                        filter(First.Prof == prof.num) %>%
                          pull(Delta2)
  return(mean(observed.delta2s))
}

##Make significance dataframe for bootstraps. The column Significant 2 notes that this significance test may create different results than the one sample t-test. We will use \alpha = 0.05 as we did for the t-test.
Sig.Profs <- as.data.frame(cbind(`Faculty Random ID` = Filtered.Profs, Significant2 = rep(NA,length(Filtered.Profs))))
Sig.Profs$Significant2 <- as.logical(Sig.Profs$Significant2)

##Create significance evaluation function to be applied against all faculty in Sig.Profs. The output will be a significance observation for one professor.
evaluate.significance <- function(prof.num){
  current.significance <- NA
  current.bootstrap.distribution <- make.overall.bootstrap.means(prof.num) %>% 
                                      pull(Delta2.Boot.Means)
  lower.quantile <- quantile(current.bootstrap.distribution, 0.025)
  upper.quantile <- quantile(current.bootstrap.distribution, 0.975)
    if(upper.quantile <= global.mean){
      current.significance <- TRUE
    } else if(lower.quantile >= global.mean){
      current.significance <- TRUE
    } else{
...
##Neither sapply nor apply wanted to work so I just manually apply the functions here. This fills out the Significant2 column in Sig.Profs.
for(i in seq_along(Sig.Profs$`Faculty Random ID`)){
  Sig.Profs$Significant2[i] <- evaluate.significance(Sig.Profs$`Faculty Random ID`[i])  
}
...
##Create visualization of a faculty member with a mean Delta2 significantly different from 0.
make.overall.bootstrap.means(sig.test.prof) %>%
  ggplot() +
  geom_histogram(aes(x = Delta2.Boot.Means)) +
  geom_vline(xintercept = c(quantile(sig.test.data$Delta2.Boot.Means, 0.025), quantile(sig.test.data$Delta2.Boot.Means, 0.975), 0), color = c("red", "red", "blue")) +
  labs(x = "Mean Delta2 from bootstrap sample", y = "Count", title = "Example Histogram of the Bootstrap Distribution for a Faculty Member with a\nSignificant Mean Delta2")

##Create visualization of a faculty member with a mean Delta2 NOT significantly different from 0.
make.overall.bootstrap.means(nonsig.test.prof) %>%
  ggplot() +
  geom_histogram(aes(x = Delta2.Boot.Means)) +
  geom_vline(xintercept = c(quantile(nonsig.test.data$Delta2.Boot.Means, 0.025), quantile(nonsig.test.data$Delta2.Boot.Means, 0.975), 0), color = c("red", "red", "blue")) +
  labs(x = "Mean Delta2 from bootstrap sample", y = "Count", title = "Example Histogram of the Bootstrap Distribution for a Faculty Member with a\nNon-Significant Mean Delta2")
```

The general bootstrap process goes as follows:

We can to simulate the sampling distribution to determine whether there is a statistically significant difference between the population Mean $\Delta2$ for all faculty members and an individual professor's observation of Mean $\Delta 2$. We can obtain a more accurate boundary by simulating the sampling distribution using bootstrapping. The general process for the bootstrapping of a population parameter $\theta$ using a given sample containing an estimate $\hat{\theta}$ follows:
...
##Determine the mean of Delta2 for the faculty that obtained differing significance from the t-test and the bootstrap
differ.mean <- Comparison.Data %>%
                filter(Sig.Agree == FALSE) %>%
                  pull(Mean.Delta2) %>%
                    mean()

##Determine the standard deviation of Delta2 for the faculty that obtained differing significance from the t-test and the bootstrap
differ.sd <- Comparison.Data %>%
              filter(Sig.Agree == FALSE) %>%
                pull(Mean.Delta2) %>%
                  sd()

##Determine the mean and standard deviation of Delta2 for the faculty that obtained the same significance from the t-test and the bootstrap (POSITIVE DELTA2)
positive.mean <- Comparison.Data %>%
                  filter(Sig.Agree == TRUE,  Mean.Delta2 > global.mean) %>%
                    pull(Mean.Delta2) %>%
                      mean()
...
##Determine the mean of Delta2 for the faculty that obtained the same significance from the t-test and the bootstrap (NEGATIVE DELTA2)
...
##Overall standard deviation for faculty that had doubly-significant deviation from the average mean Delta2
significant.sd <- Comparison.Data %>%
                    filter(Sig.Agree == TRUE, Bootstrap.Sig == TRUE) %>%
                      pull(Mean.Delta2) %>%
                        sd()

##Table of differences between t-test significance and bootstrap significance.
Comparison.Data %>%
...
## The two oddities where the t-test suggested the result was not significant, but the bootstrap suggested it was.
Comparison.Data %>%
  mutate(T.Test.Only.Reject = if_else(T.Test.Sig == FALSE & Bootstrap.Sig == TRUE, TRUE, FALSE), Bootstrap.Only.Reject = if_else(Bootstrap.Sig == FALSE & T.Test.Sig == TRUE, TRUE, FALSE)) %>%
    filter(T.Test.Only.Reject == TRUE) %>%
      dplyr::select(First.Prof, T.Test.Sig, Bootstrap.Sig, everything())
```

The above tables summarise the differences between the one sample t-test and bootstrap significance measures. We will now discuss some statistics surrounding this comparison. The significance implied by the t-test and the bootstrap agreed for `r total.agree` out of `r total.cases` faculty members, which is approximately  `r prop.agree *100`$\%$ of all professors.  In the vast majority of cases, the bootstrap significance test is more conservative than that of the t-test. This can be seen in the `r total.cases - total.agree` cases where the significance differed between the bootstrap and the t-test. $89$ discrepancies occurred where the t-test implied that the observation of mean $\Delta 2$ for a faculty member was significantly different from the overall average mean, while the bootstrap implied that it was not. Only two cases of the bootstrap failing to reject significance for a faculty member rejected by the t-test occurred, and I have included them above for the reader's consideration. In an effort to be as conservative as possible with inference, we will recreate the previous plots [CITE WHICH] using only those faculty members endorsed by the bootstrap confidence intervals.  Of the cases whose significance differed, the mean value for $\Delta 2$ was `r differ.mean` GPA points, and the standard deviation of the mean $\Delta 2$ was `r differ.sd`. Among the faculty whose significance agreed and had positive mean $\Delta 2$ values, the average mean value for $\Delta 2$ was `r positive.mean` and the standard deviation was `r positive.sd`. As for the faculty whose significance agreed but had negative mean $\Delta 2$ values, the average mean $\Delta 2$ was `r negative.mean` with a standard deviation of `r negative.sd`. The average standard deviation of $\Delta 2$ for a faculty member that had a significantly different $\Delta 2$ from the overall mean was `r significant.sd`.

```{r}
##Check the overall course distribution for each professor
