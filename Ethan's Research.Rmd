...
i <- 1
for(i in 1:n.s){
  Gen_Data$Student.Term[student.index(i)] <- student.terms(student.index(i))
}
```
...
Gen_Data <- left_join(Gen_Data, Max_terms)
```
...
Points.Tracker <- data.frame(Prof.Type = NA, Professors = Profs.List, First.Math.Points = 0, First.Chem.Points = 0, First.Bio.Points = 0, First.Withdraw.Prop = 0,  Total.Math.Points = 0, Total.Chem.Points = 0, Total.Bio.Points = 0,  Total.Withdraw.Prop = 0, Future.Withdraw.Prop = 0, Total.Students = 0, Withdraw.)
...
Course.Credits <- tibble(`Course Code` = unique(Gen_Data$`Course Code`),
...
Gen_Data_withdraw_prop <- length(Gen_Data$`GexPA Assigned`[is.na(Gen_Data$`GPA Assigned`)])/n
...
Gen_Data %>% 
      ...
      geom_point(position = position_jitter(width = 0.2, height = 0.1), alpha = 0.03)
```

  To partially answer this question, we can look at 
