...
Gen_Data <- select(Gen_Data,"Student Random ID":"GPA Assigned")
```
...
```{r}
##Create class size variable for statistical significance tests. There are 5 courses with iffy class sizes: 861, 2636, 3894, 4234, and 3148. The first four of these have n = 1 - they are independant studies. 3148 had n = 3, I'm not sure how that happened. Doing this filtration gives n >= 7, which will allow the usage of the t test. Fortunately we did not need to remove almost any cases, so validity of results is basically unaffected.
...
Plot.Data1 %>%
  ...
  labs(x = "Total withdraws", y = "Total students", title = "")  
     

  
library(lme4)
withdraws.model1 <- lmer(n_withdraws ~ total_students + (1|`Course Code`), data = Withdraws.Data)
