...
```{r Vestigial Garbage, include = FALSE}
# ##This chunk creates the Class Tracker, Mean GPA, and Delta columns in Gen_Data. This takes a while to run. I have included it in the appendix for an idea of how I was initially approaching R programming. As it happens learning more efficient methods makes things a lot more legible. Attempt to understand this chunk at your own peril. The output isn't even useful for the project, but is a bit interesting. This entire function could be replicated easily with the which() command.
# 
...
```{r Initialize Graded.Term, cache = TRUE}
...
make.course.model <- function(course){
  ...
  One.Course.Data <- Filtered.Data1 %>%
    ...
    if(length(unique(One.Course.Data$`Student Random ID`)) >= 8){
      Cum.course.model <- lmer(Cum.GPA ~ Graded.Term + (1|`Student Random ID`), data = One.Course.Data)
...
Comparison.Data <- full_join(Manual.Comparison.Data, LMER.Comparison.Data, by = c("Student Random ID", "First.Course", "First.Prof", "max.terms")) %>%
...
Comparison.Data %>%
  left_join(Course.Heights, by = c("First.Course" = "Course Code")) %>%
    ggplot(aes(x = LMER.DeltaC, y = Height)) +
    geom_boxplot(aes(color = First.Course)) +
    labs(x = "\u0394C (GPA Points)", y = "Course Code", color = "Course Code", title = "Distribution of \u0394C using LME4 Models for each Course") +
    scale_x_continuous(limits = c())

Comparison.Data %>%
  left_join(Course.Heights, by = c("First.Course" = "Course Code")) %>%
    ggplot(aes(x = Manual.DeltaC, y = Height)) +
    geom_boxplot(aes(color = First.Course)) +
    labs(x = "\u0394C (GPA Points)", y = "Course Code", color = "Course Code", title = "Distribution of \u0394C using Manual Models for each Course")
```
...
LMER.DeltaC.Data4 %>%
  dplyr::select(First.Prof, Mean.DeltaC, standard.error) %>%
    mutate(Hist.Colors = factor(if_else(Mean.DeltaC < mean(pull(LMER.DeltaC.Data4, Mean.DeltaC)), "green", "red"))) %>%
      ggplot() +
      geom_histogram(aes(x = Mean.DeltaC, fill= Hist.Colors), bins = 22) +
      labs(x = "Mean \u0394C (GPA Points)", y = "Count", title = "Histogram of T-Significant Professor Mean \u0394C's, Filled by Effect\nPolarity from the Mean") +
...
LMER.DeltaC.Data4 %>%
  filter(significant == TRUE) %>%
    dplyr::select(First.Prof, Mean.DeltaC, standard.error) %>%
      mutate(Hist.Colors = factor(if_else(Mean.DeltaC < mean(pull(filter(LMER.DeltaC.Data4, significant == TRUE), Mean.DeltaC)), "green", "red"))) %>%
        ggplot() +
        geom_histogram(aes(x = Mean.DeltaC, fill= Hist.Colors), bins = 22) +
        labs(x = "T-Significant Mean \u0394C (GPA Points)", y = "Count", title = "Histogram of T-Significant Professor Mean \u0394C's, Filled by Effect\nPolarity from the Mean") +
        theme(legend.position = "none")

##Normal probability plot for T-Significant mean DeltaC using LMER Models
LMER.DeltaC.Data4 %>%
  filter(significant == TRUE) %>%
    pull(Mean.DeltaC) %>%
     qqnorm(main = "Normal Q-Q Plot for T-Significant Mean Professor \u0394C Effects")

##Normal probability plot for All mean DeltaC using LMER Models 
LMER.DeltaC.Data4 %>%
  pull(Mean.DeltaC) %>%
   qqnorm(main = "Normal Q-Q Plot for ALL Mean Professor \u0394C Effects")

##Course Mean DeltaC Values for all students that passed filtration assumptions using LMER models
Course.DeltaC.Means <- LMER.DeltaC.Data %>%
                        group_by(`Student Random ID`) %>%
                          slice(1) %>%
                            ungroup() %>%
                              group_by(First.Course) %>%
...
##Scatterplot for only significant mean DeltaC values as measured by a one-sample t test onto each of the 23 strata using LMER Models
LMER.DeltaC.Data %>%
  ungroup() %>%
    group_by(`Student Random ID`) %>%
      slice(n()) %>%
                          ...
                          rename(`Faculty Random ID` = First.Prof, `Course Code` = First.Course) %>%
                            ungroup() %>%               
                              left_join(Course.Heights, by = "Course Code") %>%
                                ...
                                labs(x = "T-Significant Mean \u0394C (GPA Points)", y = "Class", size = "Number of Students", color = "Course Code", n = "Number of Students", title = "Scatterplot of Mean Faculty \u0394C Effects by Course\nfor T-Significant Courses") +
                                theme(axis.text.y=element_blank(), axis.ticks.y=element_blank()) +
                                scale_x_continuous(limits = c(-2,2))
           
##Scatterplot of ALL mean DeltaC values for each of the 19 strata using LMER models
LMER.DeltaC.Data %>%
  ungroup() %>%
    group_by(`Student Random ID`) %>%
      slice(n()) %>%
      ...
      group_by(First.Course) %>%
        summarise(Course.Mean.DeltaC = mean(DeltaC)) %>%
           ggplot(aes(x = fct_reorder(First.Course, -Course.Mean.DeltaC), y = Course.Mean.DeltaC)) +
            ...
            labs(x = "Course Code", y = "Course Mean \u0394C", fill = "Course Code", title = "Bar Chart of Course Mean \u0394C's using LMER Models")
...
```{r Experiment One Manual Mean DeltaC Plots}
##Boxplots of Difference between Expected and Observed Cumulative GPA as predicted by manual models for each course, colored by course code, for students that obeyed filtration assumptions
Manual.DeltaC.Data %>%
  ungroup() %>%
    group_by(`Student Random ID`) %>%
      slice(1) %>%
        ungroup() %>%
          ggplot(aes(color = First.Course)) +
          geom_boxplot(aes(x = DeltaC), alpha = 0.9) +
          labs(x = "\u0394C (GPA Points)", y = "First Course Code", color = "Course Code", title = "Boxplots of \u0394C for each Individual Student that Passed\nFiltration using Manual Models, Colored by First Course Code.") +
          theme(axis.text.y=element_blank(), axis.ticks.y=element_blank()) +
          scale_x_continuous(limits = c(-4,4))

##Histogram of ALL mean professor effects onto final cumulative GPA for students that obeyed filtration assumptions from experiment one. This graph is just to put the previous one in context, not for inference.
Manual.Experiment.One.Mean.DeltaCs %>%
  dplyr::select(First.Prof, Mean.DeltaC, standard.error) %>%
    mutate(Hist.Colors = factor(if_else(Mean.DeltaC < mean(pull(Manual.Experiment.One.Mean.DeltaCs, Mean.DeltaC)), "green", "red"))) %>%
      ggplot() +
      geom_histogram(aes(x = Mean.DeltaC, fill= Hist.Colors), bins = 22) +
      labs(x = "Mean \u0394C (GPA Points)", y = "Count", title = "Histogram of T-Significant Professor Mean \u0394C's, Filled by Effect\nPolarity from the Mean") +
      theme(legend.position = "none")
...
Manual.Experiment.One.Mean.DeltaCs %>%
  filter(Significant == TRUE) %>%
    dplyr::select(First.Prof, Mean.DeltaC, standard.error) %>%
      mutate(Hist.Colors = factor(if_else(Mean.DeltaC < mean(pull(filter(Manual.Experiment.One.Mean.DeltaCs, Significant == TRUE), Mean.DeltaC)), "green", "red"))) %>%
        ggplot() +
        geom_histogram(aes(x = Mean.DeltaC, fill= Hist.Colors), bins = 22) +
        labs(x = "T-Significant Mean \u0394C (GPA Points)", y = "Count", title = "Histogram of T-Significant Professor Mean \u0394C's, Filled by Effect\nPolarity from the Mean") +
        theme(legend.position = "none")

##Normal probability plot for T-Significant mean DeltaC using Manual Models
Manual.Experiment.One.Mean.DeltaCs %>%
  filter(Significant == TRUE) %>%
    pull(Mean.DeltaC) %>%
     qqnorm(main = "Normal Q-Q Plot for T-Significant Mean Professor \u0394C Effects from \nManual Experiment One Models")

##Normal probability plot for All mean DeltaC using LMER Models 
Manual.Experiment.One.Mean.DeltaCs %>%
  pull(Mean.DeltaC) %>%
   qqnorm(main = "Normal Q-Q Plot for ALL Mean Professor \u0394C Effects from \nManual Experiment One Models")


```

```{r Experiment Two Manual Mean DeltaC Plots}
##Histogram of ALL mean professor effects onto final cumulative GPA for students that obeyed filtration assumptions from experiment one. This graph is just to put the previous one in context, not for inference.
Manual.Experiment.Two.Mean.DeltaCs %>%
  dplyr::select(First.Prof, Mean.DeltaC, standard.error) %>%
    mutate(Hist.Colors = factor(if_else(Mean.DeltaC < mean(pull(Manual.Experiment.Two.Mean.DeltaCs, Mean.DeltaC)), "green", "red"))) %>%
      ggplot() +
      geom_histogram(aes(x = Mean.DeltaC, fill= Hist.Colors), bins = 22) +
...
Manual.Experiment.Two.Mean.DeltaCs %>%
  filter(Significant == TRUE) %>%
    dplyr::select(First.Prof, Mean.DeltaC, standard.error) %>%
      mutate(Hist.Colors = factor(if_else(Mean.DeltaC < mean(pull(filter(Manual.Experiment.Two.Mean.DeltaCs, Significant == TRUE), Mean.DeltaC)), "green", "red"))) %>%
        ggplot() +
        geom_histogram(aes(x = Mean.DeltaC, fill= Hist.Colors), bins = 22) +
        labs(x = "T-Significant Mean \u0394C (GPA Points) among those students whom started with a professor and passed filtration", y = "Count", title = "Histogram of T-Significant Professor Mean \u0394C's, Filled by Effect\nPolarity from the Mean") +
        theme(legend.position = "none")
...
##Scatterplot for only significant mean DeltaC values by course as measured by a one-sample t test onto each of the 23 strata using Manual models
Manual.Experiment.Two.Mean.DeltaCs %>%
  dplyr::select(First.Prof, First.Course, Mean.DeltaC, Num.Students, Significant) %>%
    left_join(Course.Heights, by = c("First.Course" = "Course Code")) %>%
      ggplot() +
      geom_point(aes(x = Mean.DeltaC, y = Height, color = First.Course, size = Num.Students), position = position_jitter(height = 0.005)) +
      labs(x = "T-Significant Mean \u0394C (GPA Points)", y = "Class", size = "Number of Students", color = "Course Code", n = "Number of Students", title = "Scatterplot of T-Significant Mean Faculty \u0394C Effects by Course\nusing Manual Models for Experiment Two") +
      scale_x_continuous(limits = c(-2,2))
  ...
  dplyr::select(First.Prof, First.Course, Mean.DeltaC, Num.Students, Significant) %>%
    left_join(Course.Heights, by = c("First.Course" = "Course Code")) %>%
      filter(Significant == TRUE) %>%
        ggplot() +
        geom_point(aes(x = Mean.DeltaC, y = Height, color = First.Course, size = Num.Students), position = position_jitter(height = 0.005)) +
        labs(x = "T-Significant Mean \u0394C (GPA Points)", y = "Class", size = "Number of Students", color = "Course Code", n = "Number of Students", title = "Scatterplot of ALL Mean Faculty \u0394C Effects by Course\nusing Manual Models for Experiment Two") +
        scale_x_continuous(limits = c(-2,2))
      ...
      group_by(First.Course) %>%
        summarise(Course.Mean.DeltaC = mean(DeltaC)) %>%
           ggplot(aes(x = fct_reorder(First.Course, -Course.Mean.DeltaC), y = Course.Mean.DeltaC)) +
            theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()) +
            geom_bar(aes(fill = First.Course), stat = "identity") +
            labs(x = "Course Code", y = "Course Mean \u0394C", fill = "Course Code",  title = "Bar Chart of Course Mean \u0394C's using Manual Models")
```
                                            ...
                                            slice(1) %>%
                                              ungroup() %>%
                                                filter(max.terms == term) %>%
                                                  pull(max.terms) %>%
                                                    length()
                        prop.students <- n.students/(length(unique(Gen_Data9$`Student Random ID`)))
...
## Determine the total effect faculty had in the math / chemistry departments of OCC between 2010 to 2017. Overall.Points ranks faculty based on total effect as a function of Mean DeltaC and number of students that passed filtration and started with the faculty. That is, Overall.Points is the results frame for Experiment One.
...
##Apply ranker function onto Overall.Points1
Overall.Points2 <- Overall.Points1 %>%
                    Rank.Overall.Stats()
head(Overall.Points2)

## Just as we created a results frame for Experiment One, the below creates a results frame for Experiment Two.
...
Reported.Courses <- Course.Points1 %>%
                pull(First.Course) %>%
                  unique()

##Create a ranker function that accounts for course means
Rank.Course.Stats <- function(data){
  Temp.Data <- Course.Points1 %>%
                arrange(First.Course, desc(Course.Cum.GPA.Impact))
  
##Helper function that determines which courses are the desired course. This could also be accomplished with a filtration.
  get.course.indecies <- function(course){
    return(which(Temp.Data$First.Course == course))
  }
  ##Rank faculty by Total impact by course
  for(i in 1:length(Reported.Courses)){
    course.indecies <- get.course.indecies(Reported.Courses[i])
    cntr <- 1
  ...
  ##Rank faculty by mean DeltaC against other faculty that taught the same course
  Temp.Data <- Temp.Data %>%
...
##Apply function onto Course.Points1
...
Overall.Points2 %>%
  ...
  labs(x = "Mean \u0394C (GPA Points)", y = "Cumulative GPA Impact (GPA Points)", title = "Mean \u0394C Vs. Overall Cumulative GPA Impact for Experiment One", color = "Number of Students")

Course.Points2 %>%
  ggplot(aes(x = Mean.DeltaC, y = Course.Cum.GPA.Impact)) +
  geom_point(aes(color = Num.Students)) +
  scale_x_continuous(limits = c(-1.1,1.1), breaks = seq(from = -1, to = 1, by = 1)) +
  labs(x = "Mean \u0394C (GPA Points)", y = "Cumulative GPA Impact (GPA Points)", title = "Mean \u0394C Vs. Overall Cumulative GPA Impact for Experiment Two", color = "Number of Students")
...
Overall.Points2 %>%
  ...
  labs(x = "Number of Students", y = "Cumulative GPA Impact (GPA Points)", title = "Number of Students Vs. Course Code Cumulative GPA Impact\nfor Experiment Two, Colored by Mean \u0394C")

Course.Points2 %>%
  ggplot(aes(x = Mean.DeltaC, y = Num.Students)) +
  geom_point(aes(color = First.Course)) +
  scale_x_continuous(limits = c(-1.2,1.2), breaks = seq(from = -1, to = 1, by = 1)) + 
  labs(x = "Mean \u0394C (GPA Points)", y = "Number of Students", title = "Mean \u0394C Vs. Number of Students for Experiment Two,\nColored by Course Code", color = "Course Code")
```
...
Overall.DeltaW.Data %>%
  mutate(Hist.Colors = if_else(Observed.Withdraw.Prop > Overall.Withdraw.Rate, "green", "red")) %>%
    ...
    theme(legend.position = "none") +
    labs(x = "\u0394W", y = "Count", title = "Histogram of ALL Differences between Expected and Observed\nFaculty Withdraw Rate for Experiment One")
...
Overall.DeltaW.Data %>%
  filter(Significant == TRUE) %>%
    mutate(Hist.Colors = if_else(Observed.Withdraw.Prop > Overall.Withdraw.Rate, "green", "red")) %>%
      ...
      geom_histogram(binwidth = 0.05) + 
      theme(legend.position = "none") + 
      labs(x = "\u0394W", y = "Count", title = "Histogram of Significant Differences Between Expected and Observed Faculty\nWithdraw Rate for Experiment One")

##Normal probability plot of DeltaW over all faculty for experiment one
Overall.DeltaW.Data %>%
  pull(Overall.DeltaW) %>%
    qqnorm(main = "Normal Probability Plot of \u0394W Over All Faculty for Experiment One")

##Normal probability plot of DeltaW over significant faculty for experiment one
Overall.DeltaW.Data %>%
  filter(Significant == TRUE) %>%
    pull(Overall.DeltaW) %>%
      qqnorm(main = "Normal Probability Plot of \u0394W Over Significant for Experiment One")

##Number of Faculty represented in above histogram compared to total
Gen_Data10 %>%
...
Course.DeltaW.Data %>%
  mutate(Hist.Colors = if_else(Observed.Withdraw.Rate > Course.Withdraw.Rate, "green", "red")) %>%
    ...
    geom_histogram(binwidth = 0.05) + 
    theme(legend.position = "none") %>%
    labs(x = "\u0394W", y = "Count", title = "Histogram of Differences between Expected and Observed Faculty Withdraw Rate")
...
##Normal Probability Plot for all faculty in experiment two
Course.DeltaW.Data %>%
  pull(Course.DeltaW) %>%
    qqnorm(main = "Normal Probability Plot of ALL \u0394W's for Experiment Two")

##Normal Probability Plot for significant faculty in experiment two
Course.DeltaW.Data %>%
  filter(Significant == TRUE) %>%
    pull(Course.DeltaW) %>%
      qqnorm(main = "Normal Probability Plot of \u0394W's for Experiment Two")
...
```{r DeltaW Results}
##Experiment One
##Initialize Experiment One Results Frame
Overall.Points3 <- Overall.DeltaW.Data %>%
                    filter(Significant == TRUE) %>%
                      dplyr::select(`Faculty Random ID`, Overall.DeltaW, Num.Students) %>%
                        mutate(Overall.Withdraw.Impact = Overall.DeltaW * Num.Students, Overall.DeltaW.Rank = -1, Overall.Impact.Rank = -1) %>%
                          rename(DeltaW = Overall.DeltaW)

##Create single use ranker function for faculty that had significantly different withdraw rates from the mean
Rank.Overall.Stats2 <- function(data){
  Temp.Data <- data %>%
                arrange(desc(Overall.Withdraw.Impact))
  n.sig.profs <- length(Temp.Data$`Faculty Random ID`)
  for(i in 1:n.sig.profs){
    Temp.Data$Overall.Impact.Rank[i] <- i
  }
  Temp.Data <- Temp.Data %>%
                arrange(desc(DeltaW))
  for(i in 1:n.sig.profs){
    Temp.Data$Overall.DeltaW.Rank[i] <- i
  }
  return(Temp.Data)
}
##Apply ranker function onto Overall.Points3
Overall.Points4 <- Overall.Points3 %>%
                    Rank.Overall.Stats2()
##Print Experiment One Results
head(Overall.Points4)

##Experiment Two
##Initialize Experiment Two Results Frame
Course.Points3 <-  Course.DeltaW.Data %>%
                    filter(Significant == TRUE) %>%
                      dplyr::select(`Faculty Random ID`, `Course Code`, Course.DeltaW, Num.Students) %>%
                        mutate(Course.Withdraw.Impact = Course.DeltaW * Num.Students, Course.DeltaW.Rank = -1, Course.Impact.Rank = -1) %>%
                          rename(DeltaW = Course.DeltaW)

##Create a ranker function that accounts for course means
Rank.Course.Stats2 <- function(data){
  Temp.Data <- data %>%
                arrange(`Course Code`, desc(Course.Withdraw.Impact))
  
##Helper function that determines which courses are the desired course. This could also be accomplished with a filtration.
  get.course.indecies <- function(course){
    return(which(Temp.Data$`Course Code` == course))
  }
  ##Rank faculty by Total impact by course
  for(i in 1:length(Reported.Courses)){
    course.indecies <- get.course.indecies(Reported.Courses[i])
  ...
  ##Rank faculty by DeltaW against other faculty that taught the same course
  Temp.Data <- Temp.Data %>%
                  arrange(`Course Code`, desc(DeltaW))
  for(i in 1:length(Reported.Courses)){
    course.indecies <- get.course.indecies(Reported.Courses[i])
    cntr <- 1
      for(k in course.indecies){
        Temp.Data$Course.DeltaW.Rank[k] <- cntr 
        cntr <- cntr + 1
      }
  }
...
##Mean DeltaC Vs. DeltaW for only those faculty with a significant mean DeltaC and DeltaW.
Double.Profs %>%
  mutate(SigC = if_else(T.Test.Sig == TRUE & Bootstrap.Sig == TRUE, TRUE, FALSE)) %>%
    dplyr::select(First.Prof, Mean.DeltaC, SigC) %>%
        full_join(Overall.DeltaW.Data, by = c("First.Prof" = "Faculty Random ID")) %>%
          rename(SigW = Significant) %>%
            dplyr::select(First.Prof, Mean.DeltaC, Overall.DeltaW, SigC, SigW) %>%
              mutate(Significance = factor(if_else(SigC == TRUE & SigW == TRUE, "Both \u0394C and \u0394W", if_else(SigC == FALSE & SigW == TRUE, "Only \u0394W", if_else(SigC == TRUE & SigW == FALSE, "Only \u0394C", if_else(SigC == FALSE & SigW == FALSE, "Neither \u0394C nor \u0394W", as.character(NA))))))) %>%
                filter(Significance == "Both \u0394C and \u0394W") %>%
                  ggplot(aes(x = Mean.DeltaC, y = Overall.DeltaW)) +
                  geom_point(aes(color = Significance)) + 
                  geom_smooth() + 
                  labs(x = "Mean \u0394C (GPA Points)", y = "\u0394W", color = "Significance", title = "Mean \u0394C Vs. \u0394W for Faculty with Significant Effects\nin Experiment One")
...
##Mean DeltaC Vs. DeltaW for faculty with a significant mean DeltaC and DeltaW.
Double.Courses %>%
  mutate(SigC = if_else(T.Sig == TRUE & Bootstrap.Sig == TRUE, TRUE, FALSE)) %>%
    dplyr::select(First.Prof, First.Course, Mean.DeltaC, SigC) %>%
      full_join(Course.DeltaW.Data, by = c("First.Prof" = "Faculty Random ID", "First.Course" = "Course Code")) %>%
