...
```{r}
##Create class size variable for statistical significance tests. There are 5 courses with iffy class sizes: 861, 2636, 3894, 4234, and 3148. The first four of these have n = 1 - they are independent studies. 3148 had n = 3, I'm not sure how that happened. Doing this filtration gives n > 7, which will allow for safe usage of the t test and bootstrap. Fortunately we did not need to remove almost any cases, so validity of results is basically unaffected.
...
Gen_Data4 <- left_join(Good.Class.Size, Gen_Data3, by = c("Class Tracker", "Course Code")) %>%
...
## Store student's first course GPA to Filtered Data
      ...
      group_by(`Student Random ID`) %>%
        slice(n()) %>%
          filter(!is.na(First.Prof)) %>%
            dplyr::select(First.Course, DeltaC) %>%
              ungroup() %>%
                ggplot(aes(color = First.Course)) +
                geom_boxplot(aes(x = DeltaC), alpha = 0.9) +
                labs(x = "\u0394C (0-4 scale)", y = "Class", color = "Course Code", title = "Boxplots of \u0394C for each Individual Student that Passed\nFiltration, Colored by First Course Code.") +
  ...
  filter(significant == TRUE) %>%
    dplyr::select(First.Prof, Mean.DeltaC, standard.error) %>%
      mutate(Hist.Colors = factor(if_else(Mean.DeltaC < mean(pull(filter(DeltaC.Data3, significant == TRUE), Mean.DeltaC)), "green", "red"))) %>%
        ...
        labs(x = "Mean \u0394C (0-4 scale) among those students whom started with a professor and passed filtration", y = "Count", title = "Histogram of SIGNIFICANT Mean Professor \u0394C Effects, Filled by Effect\nPolarity from the Mean") +
...
DeltaC.Data3 %>%
    dplyr::select(First.Prof, Mean.DeltaC, standard.error) %>%
      mutate(Hist.Colors = factor(if_else(Mean.DeltaC < overall.mean.DeltaC, "green", "red"))) %>%
        ggplot() +
        geom_histogram(aes(x = Mean.DeltaC, fill= Hist.Colors), bins = 22) +
        labs(x = "Mean \u0394C (0-4 scale) among those students whom started with a professor and passed filtration", y = "Count", title = "Histogram of ALL Mean Professor \u0394C Effects, Filled by Effect\nPolarity from the Mean") +
        theme(legend.position = "none")

##Normal probability plot for DeltaC
qqnorm(pull(filter(DeltaC.Data3, significant == TRUE), Mean.DeltaC), main = "Normal Q-Q Plot for Significant Mean Professor \u0394C Effects")
                          ...
                          rename(`Faculty Random ID` = First.Prof, `Course Code` = First.Course) %>%
                            ungroup() %>%               
                              left_join(Course.Heights, by = "Course Code") %>%
                                ...
                                labs(x = "Mean \u0394C (0-4 scale) among those students whom started\nwith a professor in a stratum", y = "Class", n = "Number of Students", color = "Course Code", n = "Number of Students", title = "Scatterplot of Mean Faculty \u0394C Effects by Course\nfor SIGNIFICANT Courses") +
                    ...
                    rename(`Faculty Random ID` = First.Prof, `Course Code` = First.Course) %>%
                      ungroup() %>%               
                        left_join(Course.Heights, by = "Course Code") %>%
                          ggplot() +
                          geom_point(aes(x = Mean.DeltaC, y = Height, color = `Course Code`, size = n), position = position_jitter(height = 0.005)) +
                          labs(x = "Mean \u0394C (0-4 scale) among those students whom started\nwith a professor in a stratum", y = "Class", size = "Number of Students", color = "Course Code", title = "Scatterplot of Mean Faculty \u0394C Effects by Course\nfor ALL Courses") +
                          scale_x_continuous(limits = c(-2,2))
...
sig.test.data %>%
  ...
  geom_vline(xintercept = c(quantile(sig.test.data$DeltaC.Boot.Means, 0.025), quantile(sig.test.data$DeltaC.Boot.Means, 0.975), 0), color = c("red", "red", "blue")) +
  labs(x = "Mean \u0394C from bootstrap sample", y = "Count", title = "Example Histogram of the Bootstrap Distribution for a Faculty Member with a\nSignificant Mean \u0394C")
...
nonsig.test.data %>%
  ggplot() +
  geom_histogram(aes(x = DeltaC.Boot.Means)) +
  geom_vline(xintercept = c(quantile(nonsig.test.data$DeltaC.Boot.Means, 0.025), quantile(nonsig.test.data$DeltaC.Boot.Means, 0.975), 0), color = c("red", "red", "blue")) +
...
total.agree <- Comparison.Data %>% 
                filter(Sig.Agree == TRUE) %>%
                  pull(Sig.Agree) %>%
                    sum()
total.cases <- length(Comparison.Data$Sig.Agree)

prop.agree <- total.agree/total.cases

##Determine the mean of DeltaC for the faculty that obtained differing significance from the t-test and the bootstrap
differ.mean <- Comparison.Data %>%
                filter(Sig.Agree == FALSE) %>%
                  pull(Mean.DeltaC) %>%
                    mean()

##Determine the standard deviation of DeltaC for the faculty that obtained differing significance from the t-test and the bootstrap
differ.sd <- Comparison.Data %>%
              filter(Sig.Agree == FALSE) %>%
                pull(Mean.DeltaC) %>%
                  sd()
...
positive.mean <- Comparison.Data %>%
                  filter(Sig.Agree == TRUE,  Mean.DeltaC > global.mean) %>%
                    pull(Mean.DeltaC) %>%
                      mean()

positive.sd <- Comparison.Data %>%
                  filter(Sig.Agree == TRUE,  Mean.DeltaC > global.mean) %>%
                    pull(Mean.DeltaC) %>%
                      sd()
...
```{r}
##Table of differences between t-test significance and overall bootstrap significance.
Comparison.Data %>%
...
##Table of differences between t-test and bootstrap for course-specific.
                        ...
                        mutate(requisite.tval = qt(0.95, Num.Students - 1), observed.tval = abs(Course.Mean.DeltaC - Mean.DeltaC)/(standard.error)) %>%
                          mutate(T.Sig = if_else(observed.tval > requisite.tval, TRUE, FALSE)) %>%
                            dplyr::select(First.Prof, First.Course, Significant3, T.Sig, Num.Students, Mean.DeltaC) %>%
                              mutate(T.Test.Only.Reject = if_else(T.Sig == FALSE & Significant3 == TRUE, TRUE, FALSE), Bootstrap.Only.Reject = if_else(T.Sig == TRUE & Significant3 == FALSE, TRUE, FALSE)) %>%
                                ungroup()
...
The first table summarizes the differences between the overall one sample t-test and bootstrap significance measures for all faculty that had at least eight students begin in their courses and pass filtration. We will now discuss some statistics surrounding this comparison. The significance implied by the t-test and the bootstrap agreed for `r total.agree` out of `r total.cases` faculty members, which is approximately  `r prop.agree *100`$\%$ of all professors. We set our $\alpha$ to be $0.05$, so this is a good sign. In the vast majority of cases, the bootstrap significance test agrees with the t-test. This can be seen in that the t-test and bootstrap agreed `r total.agree` times, and differed only in `r total.cases - total.agree` cases. $7$ discrepancies occurred where the t-test implied that the observation of mean $\Delta C$ for a faculty member was significantly different from the overall average mean, while the bootstrap implied that it was not. There were no observed cases where the t-test rejected significance while the bootstrap failed to reject. We would expect more bootstrap rejections than t-test rejections if the sample data were not sufficiently normal due to the sample sizes being too small. We required at least eight students to start with a faculty member, which helps with the minimum sample size issue, but there is still high variance. For example, faculty number $578022$ had $8$ students that passed filtration start in their courses, however just by chance every one of them ended with the same final cumulative GPA, and therefore the standard deviation in $\Delta C$ was precisely zero.

Here are some relevant statistics regarding the overall doubly significant mean $\Delta C$'s between the two measures.  Among the faculty whose significance differed, the mean value for $\Delta C$ was `r differ.mean` GPA points, and the standard deviation of the mean $\Delta C$ was `r differ.sd`. Among the faculty whose significance agreed and had positive mean $\Delta C$ values, the average mean value for $\Delta C$ was `r positive.mean` and the standard deviation was `r positive.sd`. As for the faculty whose significance agreed but had negative mean $\Delta C$ values, the average mean $\Delta C$ was `r negative.mean` with a standard deviation of `r negative.sd`. The average standard deviation of $\Delta C$ for a faculty member that had a significantly different $\Delta C$ from the overall mean was `r significant.sd`. The sample size had mean `r Comparison.Data %>% ungroup() %>% pull(num.students) %>% mean()` and standard deviation `r Comparison.Data %>% ungroup() %>% pull(num.students) %>% sd()`. The distribution of the sample size must therefore not be normal.  


The second table shows a more clear critisism of the bootstrap method for small sample size. In table one, we conglomerated all a faculty member's students into one overall rating, and only saw $7$ discrepancies between the tests. In the second experiment where a professor's individual courses are observations, the sample sizes are often too small for the bootstrap method to be accurate.

In an effort to be as conservative as possible with inference, we will now recreate the previous plots [CITE WHICH] using only those faculty members endorsed by both confidence intervals. This will not change much, because only seven additional professors are to be omitted.

```{r}
##Recreate graphs for doubly-significant faculty 

##For overall:

##Determine which faculty members had doubly significant effect onto their students.
Doubly.Sig.Profs <- Comparison.Data %>%
                      filter(Sig.Agree == TRUE, Bootstrap.Sig == TRUE) %>%
                        dplyr::select(First.Prof)
...
DeltaC.Data4 %>%
  mutate(Hist.Colors = factor(if_else(Mean.DeltaC < mean(pull(DeltaC.Data4, Mean.DeltaC)), "green", "red"))) %>%
        ggplot() +
        geom_histogram(aes(x = Mean.DeltaC, fill= Hist.Colors), bins = 22) +
        labs(x = "Mean DeltaC (0-4 scale) among those students whom started with a professor and passed filtration", y = "Count", title = "Histogram of Doubly-Significant Professor Mean DeltaC for Conglomerated\nCourses, Filled by Effect Polarity from the Mean") +
        theme(legend.position = "none")


##Normal probability plot for Doubly-Significant Mean DeltaC's
qqnorm(pull(DeltaC.Data4, Mean.DeltaC), main = "Normal Q-Q Plot for Doubly-Significant Mean Professor DeltaC Effects")

##Scatterplot for only doubly-significant mean DeltaC values 
Filtered.Data5 %>%
  ungroup() %>%
    group_by(`Student Random ID`) %>%
      slice(n()) %>%
                ...
                filter(n >= 8) %>%
                  left_join(Course.DeltaC.Means, by = "First.Course") %>%
                    mutate(requisite.tval = qt(0.95, n-1), observed.tval = abs(Mean.DeltaC - Course.Mean.DeltaC)/(Course.Sd.DeltaC/sqrt(n))) %>%
                      mutate(significant = if_else(observed.tval > requisite.tval, TRUE, FALSE)) %>%
                        ungroup() %>%               
                          left_join(dplyr::select(Sig.Courses, -Mean.DeltaC), by = c("First.Course", "First.Prof")) %>%
                            rename(`Faculty Random ID` = First.Prof, `Course Code` = First.Course) %>%
                              left_join(Course.Heights, by = "Course Code") %>%
                                filter(significant == TRUE, Significant3 == TRUE) %>%
                                ggplot() +
                                geom_point(aes(x = Mean.DeltaC, y = Height, color = `Course Code`, size = n), position = position_jitter(height = 0.005)) +
                                labs(x = "Mean \u0394C (0-4 scale) among those students whom started\nwith a professor in a stratum", y = "Class", n = "Number of Students", color = "Course Code", title = "Scatterplot of Mean Faculty \u0394C Effects by Course\nfor Doubly-Significant Courses") +
                                scale_x_continuous(limits = c(-2,2))

## Histogram of Sample size for faculty over all courses
DeltaC.Data4 %>%
  ungroup() %>%
    ggplot(aes(x = sample.size)) +
    geom_histogram() +
    geom_vline(xintercept = median(DeltaC.Data4$sample.size), color = "red") +
    labs(x = "Number of Students that passed filtration and started with a specific professor", y = "Count", title = "Histogram of Sample Size for Conglomerated Courses. Red Line is Overall\nMedian.")

##For Course-Specific:
##Scatterplot of course specific mean DeltaC's
Double.Courses %>%
  filter(T.Sig == TRUE, Significant3 == TRUE) %>%
    left_join(Course.Heights, by = c("First.Course" = "Course Code")) %>%
      ...
      labs(x = "Mean \u0394C (GPA Points)", y = "Course", title = "Scatterplot of All Faculty Doubly-Significant Course-Specific\nMean \u0394Cs", color = "Course Code", size = "Number of Students")
...
Double.Courses %>%
  ungroup() %>%
    ...
    geom_histogram() +
    geom_vline(xintercept = median(filter(Double.Courses, Significant3 == TRUE, T.Sig == TRUE)$Num.Students), color = "red") +
    scale_x_continuous(limits = c(0,600), breaks = seq(from = 0, to = 600, by = 100)) +
    labs(x = "Number of Students that passed filtration and started with a specific professor", y = "Count", title = "Histogram of Sample Size for Individual Courses. Red Line is Overall Median.")
...
Overall.Points <- Comparison.Data %>%
                    filter(Sig.Agree == TRUE, Bootstrap.Sig == TRUE) %>%
                      dplyr::select(First.Prof, Mean.DeltaC, num.students) %>%
                        mutate(Total.Cum.GPA.Impact = Mean.DeltaC * num.students) %>%
                          arrange(Total.Cum.GPA.Impact)
...
p <- Term.Means %>%
      filter(Num.Students >= 30) %>%
        ...
        labs(x = "Number of Terms in Math and Chemistry Departments", y = "Mean Final Cumulative GPA", title = "Mean Final Cumulative GPA Vs. Number of Terms in the\nMath and Chemistry Departments, with 95% Confidence Interval", size = "Students in Category")
...
```{r}
##Determine each Course's 
Course.Withdraw.Rates <- Gen_Data9 %>%
                          ungroup() %>%
                            group_by(`Course Code`) %>%
...
DeltaW.Data1 <- Withdraws.Data %>%
                  group_by(`Faculty Random ID`) %>%
                    summarise(DeltaW = sum(Num.Students*Course.DeltaW)/sum(Num.Students))

```

The courses Quantitative Reasoning and Plane Geometry were only taught once each, by one faculty member. These were likely trial courses that did not prove popular/useful enough to merit becoming a mainstay at OCC. Because the courses were only taught by a single faculty member, it is meaningless to compare the effect the professor had on withdraw rate by comparing to the course average. I have therefore removed them from the following analysis.

```{r}
##Histogram of course withdraw rates. There seems to be a slight right skew, but this is likely due to variance as there only being 21 remaining courses.
Course.Withdraw.Rates %>%
  ggplot(aes(x = Course.Withdraw.Rate)) +
  geom_histogram(binwidth = 0.05) +
  labs(x = "Probability of Withdrawal", y = "Count", title = "Histogram of all Course Withdraw Rates")
...
Withdraws.Data %>%
  left_join(Course.Heights, by = "Course Code") %>%
    ggplot(aes(x = Course.DeltaW, y = Height)) +
